 ...
 */
public final class IDFilter extends Filter {
    ...
    /**
     * Creates a new {@link IDFilter} from the given list. The list
     * can contain duplicate terms and multiple fields.
     */
    public IDFilter(final List<Term> terms) {
    ...
    /**
     * Creates a new {@link IDFilter} from the given {@link BytesRef} list for
     * a single field.
     */
    public IDFilter(final String field, final List<BytesRef> terms) {
    ...
    /**
     * Creates a new {@link IDFilter} from the given {@link BytesRef} array for
     * a single field.
     */
    public IDFilter(final String field, final BytesRef...terms) {
        // this ctor prevents unnecessary Term creations
        this(field, Arrays.asList(terms));
    }

    /**
     * Creates a new {@link IDFilter} from the given array. The array can
     * contain duplicate terms and multiple fields.
     */
    public IDFilter(final Term... terms) {
        this(Arrays.asList(terms));
    }


    private IDFilter(FieldAndTermEnum iter, int length) {
        ...
        this.termsAndFields = termsAndFields.toArray(new TermsAndField[0]);
        this.hashCode = hash;
            ...
            if ((terms = fields.terms(termsAndField.field)) != null) {
                ...
                for (int i = termsAndField.start; i < termsAndField.end; i++) {
                    ...
                    if (termsEnum.seekExact(spare)) {
                        ...
                        // we don't need to search any longer since we're searching ids, and once an id is found,
                        // there is no need to keep searching
                        while (docs.nextDoc() != DocIdSetIterator.NO_MORE_DOCS) {
                            if (!Objects.isNull(result)) result.set(docs.docID());
                            break;
                        }
                    }
                }
    ...
    public String toString() {
        ...
        boolean first = true;
        for (TermsAndField current : termsAndFields) {
